...
def calc_age_years(birthdate, ref_date=None):
    if not birthdate:
        return None
    if ref_date is None:
...
# เกณฑ์ประเมินอัตโนมัติแบบใช้งานโรงเรียน: ใช้เพศ + อายุ + น้ำหนัก + ส่วนสูง
# หมายเหตุ: เป็นเกณฑ์ประมาณเพื่อช่วยกรอก/คัดกรองเร็ว หากต้องการตรงกราฟกรมอนามัย 100% สามารถนำตารางมาตรฐานมาใส่แทนชุดนี้ได้
HEIGHT_MEDIAN = {
    'ชาย': {11:143,12:149,13:156,14:163,15:169,16:171,17:172,18:173},
    'หญิง': {11:146,12:152,13:155,14:157,15:158,16:158,17:159,18:159},
}
WEIGHT_MEDIAN = {
    'ชาย': {11:35,12:40,13:46,14:52,15:57,16:61,17:64,18:66},
    'หญิง': {11:37,12:41,13:45,14:48,15:50,16:51,17:52,18:53},
}

def nearest_age(age):
    if age is None:
        return None
    return min(max(int(round(age)), 11), 18)

def evaluate_growth(gender, birthdate, weight, height):
    """คืนค่า (นน.อายุ, สส.อายุ, นน.สส./ภาวะโภชนาการ) จากข้อมูลที่มี"""
    weight = parse_float(weight)
    height = parse_float(height)
    age = calc_age_years(birthdate)
    g = gender if gender in ('ชาย', 'หญิง') else 'ชาย'
    a = nearest_age(age)
    wa = ''
    ha = ''
    wh = ''
    if a and weight:
        med_w = WEIGHT_MEDIAN[g].get(a)
        if weight < med_w * 0.85:
            wa = 'น้ำหนักน้อยกว่าเกณฑ์'
        elif weight <= med_w * 1.20:
            wa = 'น้ำหนักตามเกณฑ์'
        else:
            wa = 'น้ำหนักมากกว่าเกณฑ์'
    if a and height:
        med_h = HEIGHT_MEDIAN[g].get(a)
        if height < med_h - 10:
            ha = 'เตี้ย'
        elif height < med_h - 5:
            ha = 'ค่อนข้างเตี้ย'
...
def auto_update_growth_for_student(con, sid, actor='system'):
    row = con.execute('SELECT * FROM students WHERE id=?', (sid,)).fetchone()
    if not row:
        return False
    wa, ha, wh = evaluate_growth(row['gender'], row['birthdate'], row['weight'], row['height'])
    con.execute("""UPDATE students
                   SET growth_weight_age=?, growth_height_age=?, growth_weight_height=?, nutrition=?, updated_at=?, updated_by=?
                   WHERE id=?""",
...
def save_student(sid):
    ...
    if request.method=='POST':
        ...
        if auto_wa:
            data['growth_weight_age'] = auto_wa
        if auto_ha:
            data['growth_height_age'] = auto_ha
        if auto_wh:
            data['growth_weight_height'] = auto_wh
            data['nutrition'] = auto_wh
...
@app.route('/students/recalculate-growth', methods=['POST'])
@login_required
def recalculate_growth():
    rid = request.form.get('round_id', type=int)
    ...
    if room:
        ...
        params.append(room)
    if q:
        sql += ' AND (id_card LIKE ? OR name LIKE ?)'
        params += [f'%{q}%', f'%{q}%']
    rows = con.execute(sql, params).fetchall()
    count = 0
    for row in rows:
        sid = row['id']
        if auto_update_growth_for_student(con, sid, current_user() or 'system'):
            count += 1
    con.commit()
    con.close()
    log('recalculate_growth', f'{count} records')
    flash(f'ประเมินภาวะการเจริญเติบโตอัตโนมัติแล้ว {count} รายการ', 'success')
    return redirect(url_for('students', round_id=rid or '', grade=grade, room=room, q=q))
